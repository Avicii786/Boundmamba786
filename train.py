...
class BoundNeXtLightning(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if batch_idx % 100 == 0 and self.trainer.is_global_zero: print(".", end="", flush=True)
        return loss

    def validation_step(self, batch, batch_idx):
        t1, t2, l1, l2, gt_cd = batch['img_A'], batch['img_B'], batch['sem1'], batch['sem2'], batch['bcd']
        pred_ss1, pred_ss2, pred_cd, _ = self(t1, t2) # inference mode aligns tasks automatically
        loss, _ = self.criterion((pred_ss1, pred_ss2, pred_cd, extract_boundary(gt_cd)), (l1, l2, gt_cd, extract_boundary(gt_cd)))
    ...
    def on_validation_epoch_end(self):
        ...
        if self.trainer.is_global_zero and self.trainer.state.stage != 'sanity_check':
    ...
    def configure_optimizers(self):
        ...
        # Cosine Annealing with Warm Restarts is better for escaping local minima
...
def main():
    ...
    parser.add_argument('--model_type', type=str, default='convnextv2_base') # Upgraded to Base
    parser.add_argument('--weights', type=str, default=None)
    parser.add_argument('--batch_size', type=int, default=4)
    parser.add_argument('--epochs', type=int, default=150)
    parser.add_argument('--freeze_epochs', type=int, default=5)
    parser.add_argument('--lr', type=float, default=1e-4) # Reduced LR
    ...
    trainer = pl.Trainer(
        ...
        sync_batchnorm=True # SOTA REQUIREMENT: Fixes DDP statistics destruction
    )
